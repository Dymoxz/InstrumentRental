...
@Component({
  selector: 'lib-rental-lending-out',
  templateUrl: './rental.lending-out.component.html',
})
export class RentalLendingOutComponent implements OnInit {
  rentals: IRental[] = [];
  ...
  if (token) {
    ...
    this.rentalService.getByOwnerEmail(email).subscribe(
      (rentals: IRental[]) => {
