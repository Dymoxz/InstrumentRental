...
export class InstrumentService {
  ...
  private instruments$ = new BehaviorSubject<IInstrument[]>([
    ...
    },
    {
      ...
      description:
        'A professional-grade Yamaha U3 Upright Piano available for hire. This piano has been lovingly cared for and has a polished ebony finish that looks stunning in any setting. It delivers a rich, resonant tone and smooth key action, making it perfect for professional performances, studio recordings, or even special events. Whether you’re an experienced pianist or looking to host a classy event, this piano won’t disappoint!',
    ...
    },
    {
      ...
      description:
        'A rare opportunity to play one of the most sought-after saxophones in the world! This Selmer Mark VI alto saxophone is renowned for its warm, rich tone and impeccable craftsmanship. Whether you’re performing jazz, classical, or even contemporary music, this instrument delivers the goods. It has been carefully maintained and is ready for your next concert or recording session.',
    ...
    },
    {
      ...
      description:
        'Looking for a powerful yet compact synth for your next project? The Korg Minilogue is a versatile polyphonic synthesizer that’s perfect for both live performances and studio recordings. With its sleek design, intuitive controls, and warm analog sound, it’s ideal for creating everything from lush pads to punchy leads. In great condition and ready to take your sound to the next level!',
      type: InstrumentType.electronic,
      brand: 'Korg',
      model: 'Minilogue',
      pricePerDay: 40,
  ...
  delete(id: string): void {
    ...
    if (current.length === updated.length) {
      throw new NotFoundException(
        `Instrument with id ${id} could not be found!`
      );
    }
  ...
  update(
    ...
    const current = this.instruments$.value;
    const index = current.findIndex((instrument) => instrument.id === id);
    if (index === -1) {
      throw new NotFoundException(
        `Instrument with id ${id} could not be found!`
      );
    }
    const instrument = current[index];
    const newInstrument = { ...instrument, ...updatedInstrument };
    current[index] = newInstrument;
    this.instruments$.next([...current]);
    return newInstrument;
  }
}
